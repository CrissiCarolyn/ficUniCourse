...
/* harmony import */ var _modules_LikePost__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(/*! ./modules/LikePost */ "./src/modules/LikePost.js");
...
const likepost = new _modules_LikePost__WEBPACK_IMPORTED_MODULE_7__["default"]();
...
class LikePost {
  ...
  async createLikePost(postCurrentLikeBox) {
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default().post(universityData.root_url + "/wp-json/university/v1/managePostLike", {
        "blogId": postCurrentLikeBox.getAttribute("data-blog")
      });
      if (response.data != "Only logged in users can create a like.") {
...
/* harmony default export */ __webpack_exports__["default"] = (LikePost);
      ...
      this.resultsDiv.innerHTML = `
        <div class="row">
          <div class="one-third">
            ...
            ${results.generalResults.length ? '<ul class="link-list min-list">' : "<p>No general results match that search.</p>"}
              ${results.generalResults.map(item => `<li><a href="${item.permalink}">${item.title}</a> ${item.postType == "post" ? `by ${item.authorName}` : ""}</li>`).join("")}
